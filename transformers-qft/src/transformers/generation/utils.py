        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if False:
                ...
                print("前10个最大概率:")
                for i in range(top_k):
                    print(f"索引 {top_indices[0][i].item()}: {top_probs[0][i].item():.4f}")
                
                next_tokens = torch.tensor([77091], device=next_tokens.device)   #assistant
                next_tokens = torch.tensor([1782], device=next_tokens.device)  #the            
